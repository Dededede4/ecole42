...
let randomcard nbr = List.nth Card.all (Random.int nbr)

let havecard arr card = if ((List.find_opt (function a -> if a = card then true else false) arr) = None) then false else true

let newDeck () = 
	let rec loop nbr arr =
		let card = randomcard 52 in
		if (nbr = 52) then arr
		else if (havecard arr card = true) then loop nbr arr
		else loop (nbr + 1) ([card] @ arr)
	in loop 0 []
