...
int 	ft_pushswap_instructs_simplificator_del(t_instructs	**lst, char *str1, char *str2)
{
	t_instructs	*current;
	...
	while (current->next)
	{
		if (ft_strcmp(current->content, str1) == 0 && ft_strcmp(current->next->content, str2) == 0)
		{
			if (precedent)
