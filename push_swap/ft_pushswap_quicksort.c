...
void			ft_pushswap_quicksort_fixgoodposition_a(t_vals **fixeds, t_vals *lst, int start)
{
	...
	while (lst)
	{
		if (start-- < 0 &&*((int*)lst->content) == *((int*)sorted->content))
...
void 		ft_pushswap_quicksort_a2b(t_stacks *stacks, t_vals **f_nbrs)
{
	...
	while (stacks->stacka)
	{
		...
		ft_printf("sortie.\n");
		ft_pushswap_quicksort_fixgoodposition_a(f_nbrs, stacks->stacka, ft_lstlen(stacks->stackb));
...
void 		ft_pushswap_quicksort_b2a(t_stacks *stacks, t_vals **f_nbrs)
{
	...
	while (stacks->stackb)
	{
		...
		ft_pushswap_quicksort_fixgoodposition_b(f_nbrs, stacks->stackb, ft_lstlen(stacks->stacka));
        while (stacks->stackb && ft_pushswap_quicksort_isfixed(*f_nbrs, *((int*)stacks->stackb->content)))
               ft_pushswap_instruct(INSTRUCT_PA, stacks);
        if (ft_pushswap_quicksort_isallfixed(f_nbrs, stacks->stackb))
               return ;
